...
Geometry *geometry() {
    ...
    u16 fan_center_bottom = CYL_VERT(vec3(0.0, -0.5, 0.0)),
        fan_center_top    = CYL_VERT(vec3(0.0,  0.5, 0.0));
    for (f32 i = 0.0f; i < 10.0f; i++) {
        f32 t0 =  i         / 10.0f * TAU32,
            t1 = (i + 1.0f) / 10.0f * TAU32;
        u16 bottom_l = CYL_VERT(vec3(sinf(t0), -0.5, cosf(t0))),
            bottom_r = CYL_VERT(vec3(sinf(t1), -0.5, cosf(t1))),
               top_l = CYL_VERT(vec3(sinf(t0),  0.5, cosf(t0))),
               top_r = CYL_VERT(vec3(sinf(t1),  0.5, cosf(t1)));
...
Vec3 unproject(Vec2 point) {
    f32 w = sapp_width(), h = sapp_height();
    point.y = h - point.y;
    Vec4 res = mul4x44(invert4x4(renderer.vs_params.view_proj), vec4(
        2.0 * point.x / w - 1.0,
        2.0 * point.y / h - 1.0,
        1.0,
        1.0
    ));
    return div4f(res, res.w).xyz;
}
...
void start_render(Mat4 view) {
    ...
    Mat4 proj = perspective4x4(45.0f, w/h, 0.01f, 100.0f);
...
typedef enum { MaterialKind_Normal, MaterialKind_Ghost } MaterialKind;

void draw_asset(Mat4 mat, Asset asset, Vec4 color, MaterialKind material) {
    switch (material) {
    case (MaterialKind_Normal):;
...
void end_render() {
    ...
    for (int i = 0; d = &renderer.ghosts[i], i < renderer.ghost_count; i++)
        draw_asset(d->mat, d->asset, d->color, MaterialKind_Normal);
